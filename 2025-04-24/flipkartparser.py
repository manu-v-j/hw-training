...
class FlipkartParser:
    ...
    def fetch_html(self):
        try:
            ...
            brand_names=[product['brand'] for product in products]
            product_with_price=[product for product in products if product['price']]
            for p in product_with_price:
                print(f"{p['brand']} - {p['price']}")


            with open("flipkart_products.txt", "w") as f:
                for product in product_with_price:
